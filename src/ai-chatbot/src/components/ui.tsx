    ...
    <Card className="p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-2 text-3xl font-bold tracking-tight text-ink-900 tnum">{value}</div>
          {sub && <div className="mt-1 text-xs text-ink-400">{sub}</div>}
        </div>
        {icon && (
          <div className={"grid h-10 w-10 place-items-center rounded-xl " + ac[accent]}>{icon}</div>
        )}
      </div>
    </Card>
  );
