...
import { FRAME_COLORS } from "../data/seed";
...
// ---------------- FrameBadge(枠番) ----------------
export function FrameBadge({ frame, no, size = 26 }: { frame: number; no: number; size?: number }) {
  const c = FRAME_COLORS[frame] ?? FRAME_COLORS[1];
  return (
    <span
      className="grid shrink-0 place-items-center rounded-[5px] text-xs font-bold tnum"
      style={{
        width: size,
        height: size,
        background: c.bg,
...
export function CardStatusBadge({ status }: { status: string }) {
  const map: Record<string, { tone: keyof typeof TONE; label: string }> = {
    未生成: { tone: "gray", label: "未生成" },
    生成済: { tone: "blue", label: "生成済" },
    公開済: { tone: "green", label: "公開済" },
  };
  const m = map[status] ?? map["未生成"];
  return (
    <Pill tone={m.tone}>
...
export function ResultStatusBadge({ status }: { status: string }) {
  const map: Record<string, { tone: keyof typeof TONE; label: string }> = {
    発走前: { tone: "gray", label: "発走前" },
    確定: { tone: "amber", label: "確定・掲載待ち" },
    掲載済: { tone: "green", label: "掲載済" },
  };
  const m = map[status] ?? map["発走前"];
  return (
    <Pill tone={m.tone}>
      <StatusDot tone={m.tone} />
      {m.label}
